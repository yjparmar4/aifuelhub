...
async function main() {
    console.log('Adding AI Cybersecurity blog post...')

    // Get the Security category or create fallback
    let category = await prisma.category.findUnique({
        where: { slug: 'security' }
    })

    if (!category) {
        // Try ai-tools category as fallback
    ...
    const postData = fs.readFileSync(path.join(__dirname, 'cybersecurity-ai-blog.json'), 'utf-8')
    ...
    await prisma.blogPost.create({
        data: {
            ...
            excerpt: post.excerpt,
            content: post.content,
            coverImage: '/blog/images/cybersecurity-ai-protection-hero.png',
            focusKeyword: 'AI cybersecurity',
            metaTitle: post.metaTitle,
            metaDescription: post.metaDescription,
            category: {
